...
def test_legacy_default_instruction_does_not_leak_to_another_workflow(tmp_path, monkeypatch):
    target = tmp_path / "grok_instruction_set.json"
    ...
    with pytest.raises(ValueError, match="활성 프롬프트 지시문이 없습니다"):
        svc.resolve_workflow_instruction_set("Pickme_Workflow.json")
...
def test_delete_instruction_removes_only_the_selected_workflow_document(tmp_path, monkeypatch):
    ...
    result = svc.delete_instruction_document("1-images.json", "guide-1")
...
def test_copy_instruction_documents_creates_independent_target_workflow_set(tmp_path, monkeypatch):
    ...
    copied = svc.copy_instruction_documents("1-images.json", "Pickme_Workflow.json")

    assert copied["instructionSet"]["workflowId"] == "Pickme_Workflow.json"
    ...
    svc.save_instruction_document(
        {"workflowId": "Pickme_Workflow.json", "id": copied["items"][0]["id"], "role": "CORE", "contentMarkdown": "target core"},
        document_id=copied["items"][0]["id"],
    )
    assert "source core" in svc.resolve_workflow_instruction_set("1-images.json")["compiledMarkdown"]
    assert "target core" in svc.resolve_workflow_instruction_set("Pickme_Workflow.json")["compiledMarkdown"]
...
def test_instruction_source_workflows_only_lists_workflows_with_documents(tmp_path, monkeypatch):
    ...
    assert svc.list_instruction_source_workflows() == ["1-images.json", "inactive-only.json"]

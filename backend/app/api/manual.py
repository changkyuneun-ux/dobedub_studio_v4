...
def manual(request: Request, _: CurrentUser = Depends(require_permission("manual:read"))):
    try:
        # 매뉴얼은 사용자별 내용이 아니며, 앱 프로세스 렌더 캐시와 브라우저 단기
        # 캐시를 함께 사용한다. 인증이 필요한 응답이므로 shared cache는 허용하지 않는다.
        with request_timing(request, "render"):
            content = manual_html_page()
        return HTMLResponse(content=content, headers={"Cache-Control": "private, max-age=300"})

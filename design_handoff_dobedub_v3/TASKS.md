...
- [x] 화면의 `미구현` 배지 영역을 실제 데이터로 교체 (`3b` 변경 기록, `7c` 접근 이력, `5b` Pod 제어 이력, `4b`·`7a` 변경 이력) — *공용 `frontend/src/components/AuditLogTable.tsx` 컴포넌트를 만들어 `Create3bScreen`(targetType=role)/`Create5bScreen`(targetType=sandbox_pod)/`Create7aScreen`(targetType=prompt_system_prompt)/`PromptCatalogAdminPanelV3`(targetType=prompt_category_group)/`Create7cScreen`(actorId=해당 사용자, action=login, 신규 등록 폼일 때는 미표시)에 삽입. `AppShell.tsx` 사이드바의 `adminAuditLog` 항목에서 `unimplemented` 배지도 제거하고 신규 `AdminAuditLogScreen`(라우트 `admin.auditLog`)으로 연결. `tsc -b`/`vite build` 통과 확인. 2026-08-12: 사용자 요청으로 감사 로그를 "어드민 정보 수정사항"만 남기도록 범위를 좁혀 `action="login"`(auth.py)과 `action="history.delete"`(history.py) 기록 지점을 제거 — 남은 기록 지점은 `role.permissions.update`/`user.*`/`prompt_catalog.*`/`sandbox_pod.start·stop` 8종. `7c`의 "접근 이력" 섹션은 더 채워질 데이터가 없어 항상 빈 상태가 되므로 섹션째 제거(위 각주 목록에서 `7c` 부분 갱신 필요). 기존에 쌓여 있던 `login` 레코드 28건은 마이그레이션 `20260812_0018`로 삭제(28→0, 다른 3건은 보존). `history.delete` 기존 기록은 삭제 요청이 없어 남겨둠 - 신규 기록만 중단. `AuditLogTable`의 "작업" 컬럼도 원본 action 문자열 대신 짧은 한글 라벨("사용자 수정" 등)로 표시해 폭을 100px로 줄이고, 확보한 공간을 "상세"(JSON 보기) 컬럼에 배분(`minmax(280px,1fr)`) - 기존엔 상세 칸이 80px 고정이라 JSON이 가로 스크롤까지 생겼음. 2026-08-12(같은 날 후속): 위 조정이 과했다는 사용자 피드백 - "상세"를 유동 폭으로 넓히자 이번엔 "대상"(target) 칸이 160px 고정으로 좁아져 `history_item · task_20260805_112339_f5def9` 같은 값이 두 줄로 줄바꿈됨(스크린샷으로 확인). `gridColumns`를 `"170px 110px 100px 160px minmax(280px,1fr)"` → `"170px 110px 100px minmax(220px,1fr) 160px"`로 재조정 - 유동 폭을 상세→대상으로 옮기고, 상세는 80px보다는 넉넉한 고정 160px로 유지.*
...
- [x] A-04의 `audit_logs`에 `action='login'`으로 흡수. 별도 테이블 만들지 마십시오. — *`backend/app/api/v1/auth.py`의 `login()`에서 성공/실패 모두 `action="login"`으로 기록(실패 시 비밀번호 등 민감정보는 남기지 않고 사유 메시지만). `7c` 사용자 상세 화면에서 해당 사용자의 로그인 이력으로 노출. 2026-08-12: 사용자 요청으로 감사 로그를 "어드민 정보 수정사항"만 남기도록 범위를 좁히며 이 접근 이력 기능 자체를 폐지 - `login()`이 더 이상 `record_audit_log`를 호출하지 않고, `7c`의 "접근 이력" 섹션도 제거됨. 기존 로그인 기록은 마이그레이션 `20260812_0018`로 삭제. A-04 항목 각주 참조.*
...
- [x] A-04와 함께 처리 (삭제 행위를 감사 로그에 기록) — *`history.py` 라우트가 삭제 직전 스냅샷(before_json)과 함께 `action="history.delete"`로 기록(A-04에서 완료). soft delete로 바뀐 뒤에도 그대로 동작. 2026-08-12: 사용자 요청으로 감사 로그를 "어드민 정보 수정사항"만 남기도록 범위를 좁히며 이 기록 지점 제거 - 자기 작업 이력을 지우는 건 관리자 정보 수정이 아니라는 판단. `delete_history_item` 라우트가 더 이상 `record_audit_log`/`db` 세션을 쓰지 않도록 정리. 기존에 쌓여 있던 `history.delete` 레코드는 삭제 요청이 없어 보존.*
...
- [x] **C-01 `2b` 프롬프트 경고 표시** — `generate_prompt`가 이미 용어 검증·관계 적용·`prompt_rules` 평가·Scene 검증을 거쳐 `{code, message, severity}` 배열을 반환하고 `warnings_json`에 저장합니다. 화면은 이를 심각도별로 나눠 그리기만 하면 됩니다. — *경고 심각도별 그룹핑 로직은 완료(커밋 `9535b9f`, `e048f72`)했으나, 구버전 `PromptBuilderModal`(기존 dark 테마) 안에 구현되어 README Design Tokens를 적용하지 않음. 신규 `2b` 화면(E-02)에서 로직만 재사용하고 UI는 새로 그려야 함. 참고 — 이 커밋은 error 심각도일 때 Apply 버튼을 실제로 비활성화하는 동작을 TASKS.md 문구("그리기만 하면 됩니다")보다 넓게 추가했음(라벨-동작 불일치 방지 목적) — E-02 재구현 시 이 동작도 유지. 2026-08-12: 사용자 버그 리포트(4개 항목 중 1번) 대응 — `prompt_llm_client.py`의 `_json_request()`에서 502/503/504/URLError 재시도 횟수를 2회→4회(백오프 3·5·8초)로 늘려 콜드스타트 시 "여러번 시도 후 성공" 문구가 자주 뜨던 문제 완화(TimeoutError 재시도 한도는 기존대로 별도 유지). 2026-08-12(같은 리포트 2번): `build_scene_json()`이 `_validate_and_normalize_terms()`를 용어 관계 적용 전/후 두 번 호출하며 같은 경고("SUBJECT_TYPE is required..." 등)가 rule-check 패널에 중복 표시되던 문제 수정 — `_dedupe_warnings()` 헬퍼((code, message) 키)를 합산 경고 목록에 적용.*
...
- [x] **C-05 `4c` 재사용** — `GET /api/prompts/reusable` (keyword·workflowId·minRating·reviewedOnly·reuseEligible·page·pageSize) 존재. — *E-03(금번 세션, 커밋 예정)에서 `Create4cScreen` 구현으로 반영. 2026-08-11: 카드 그리드 → 리스트 + 서버사이드 페이지네이션(20건 고정) 요청으로 `limit` 파라미터를 `page`/`pageSize`로 교체하고 응답을 `{items,page,pageSize,total}` 규격으로 통일(`/api/history`·`/api/admin/audit-logs`와 동일 규격). 이전엔 키워드 검색 시 200건까지만 조회해 필터링한 뒤 앞부분만 반환해 뒤쪽 결과가 조용히 누락될 수 있던 버그도 함께 해소. 2026-08-11(같은 날 후속): (1) "라이브러리 재사용" 버튼을 거치지 않고 이 화면에 직접 들어오면 목록이 비어 Search를 눌러야 했던 문제 수정 - 다른 `review.*` 라우트와 동일하게 `StudioShell`의 라우트 진입 이펙트에서 자동 조회하도록 추가. (2) 정보 컬럼을 워크플로/시작·다음 이미지/프롬프트(Positive)/프롬프트(Negative)/사유/코멘트/레이팅/생성자/모델명 9개로 재구성 - "생성자"는 기존에 API가 내려주지 않던 값이라 `task_tracking_service.py`가 `TaskPrompt.task.user`를 selectinload로 조인해 `createdBy`(작업을 제출한 사용자 이름)를 새로 추가했고, "시작·다음 이미지"는 이미 있던 `inputAssets[0]`/`inputAssets[1]`을 키프레임 썸네일로 보여주는 것으로 - 둘 다 실제 DB 값이며 지어낸 데이터 없음. (3) 별도 "적용" 컬럼을 제거하고 행 전체 클릭으로 대체했다가, 곧바로 사용자 요청으로 "적용" 버튼 컬럼을 다시 복구함(맨 오른쪽 컬럼, 기존과 동일하게 버튼 클릭으로만 적용). (4) 프롬프트 셀은 한 줄 말줄임 대신 고정 높이 스크롤 박스로 전체 내용을 볼 수 있게 함.*
...
- [x] `frontend/src/components/`에 사이드바 212px + 헤더 + 본문 그리드 + 우측 패널을 담는 레이아웃 컴포넌트 신설 — 화면마다 새로 짜지 않고 전 화면이 공유 — *`frontend/src/components/AppShell.tsx` 신설. 2026-08-12: 사용자 요청 2건 — (1) "관리자 콘솔 →"/"← 스튜디오" 전환 버튼을 사이드바 최하단 계정 블록에서 로고 바로 아래·GENERATE/ADMIN 라벨 위(`.v3-sidebar-switch-top`)로 이동, 더 눈에 띄게. (2) GENERATE/ADMIN/HELP 그룹 라벨과 하위 메뉴 항목의 들여쓰기가 거의 같아(16px vs 18px) 계층이 안 보이던 문제 - `.v3-sidebar-nav` 좌측 여백을 늘려 항목 텍스트가 라벨보다 약 14px 더 오른쪽에서 시작하게 하고, 그룹 라벨 font-weight를 500→700으로 올려 대비를 더함. 2026-08-12(같은 세션 후속): 사용자 요청 - "메뉴 별 하위 정보 구분(탭수준 계층 메뉴) 위치 조정 및 표준화, 화면마다 라벨 크기·강조가 다름". 화면별 `sidebarExtra` 11곳(스텝 트래커·필터 목록·중첩 트리 전부 포함)을 전수 조사한 뒤 세 가지로 표준화: (1) `AppShell.tsx`의 `.v3-sidebar-extra`에 `border-top`+`padding-top`을 추가해 상단 GENERATE/ADMIN 메뉴와 화면별 하위 정보 사이에 항상 구분선이 보이도록 공통화(개별 화면에서 각자 여백을 주던 것을 제거). (2) 라벨이 아예 없던 4개 화면(`Create4aScreen`/`Create6dScreen`/`Create2aScreen`/`Create2fScreen`)에 `LABEL · N` 형식의 `.v3-label` 라벨 추가. (3) 기존 라벨 문구를 전부 "영문 대문자 · 건수" 형식으로 통일(예: "필터 · 현재 페이지 N건" → "FILTER · N", "Collection · N" → "COLLECTION · N") - 화면마다 한글/영문·구두점이 제각각이라 계층 인지가 어렵다는 지적 반영. 카탈로그 트리(`PromptCatalogAdminPanelV3`) 최상위 레벨이 13px/700으로 다른 화면 표준(12px/600)보다 커서 별도로 튀어 보였던 것도 12px/600으로 맞추고, 하위 두 레벨(11.5px/500, 11px/500)과의 크기 단계도 함께 재정렬.*
...
- [x] `2b` 프롬프트 구성 — **C-01 경고 그룹핑 로직(심각도별 분류, BLOCK 시 Apply 비활성) 재사용, UI는 신규 토큰·레이아웃으로 재구현.** 경고는 좌측 본문 상단 스트립에 모으고 우측 패널로 분산하지 않음(README 지시) — *커밋 `cc85438`. 2026-08-11: 키워드 카탈로그가 그룹·서브카테고리 구분 없이 전부 펼쳐져 보이던 문제 수정 — `helpers/promptCatalog.ts`에 이미 있었지만 어디서도 쓰이지 않던 `promptGroupAccordionKey`/`promptCategoryAccordionKey`/`promptAccordionDefaultKeys`(빈 Set = 기본 전체 접힘)를 `Create2bScreen`에 연결해 그룹→서브카테고리 2단 아코디언(기본 접힘)으로 재구현. 2026-08-11(같은 날 후속): 사용자 요청 — 세그먼트가 여럿일 때 `2b`(프롬프트)↔`2e`(노드 컨피그) 화면 전환이 잦아 혼란스럽다는 지적으로 두 화면을 `Create2bScreen` 하나로 병합, 본문을 `.v3-segment-editor-columns`로 좌우 분할(왼쪽=프롬프트/키워드 카탈로그, 오른쪽=Wan Node Config). 좌측 사이드바에서 세그먼트를 선택하면 같은 화면 안에서 프롬프트·노드 컨피그를 동시에 편집. `Create2eScreen`은 삭제되고 `create.segments` 라우트도 제거(`router.ts`에서 `StudioRoute`/`ROUTE_PATH`), `create.prompt`가 두 화면의 역할을 모두 담당. 세그먼트별 키프레임 쌍 미리보기 카드는 `2f`(실행 전 확인) 상단으로 이동(아래 `2f` 각주 참조). 2026-08-12: Scene Detail(선택 입력) 필드에 안내가 전혀 없어 처음 쓰는 사용자가 뭘 적어야 할지 알기 어렵다는 지적 — 구버전 `main.tsx` 3019~3037행에 있던 용도 설명·권장 입력 순서("대상/관계 → 주요 동작 → 보조 동작·상호작용 → 카메라 → 표현·분위기")·구조화된 placeholder·접이식 "입력 예시 보기" 안내를 그대로 이관. 자유 텍스트 필드라 형식을 지키지 않아도 Generate Prompt 동작에는 영향 없음(순전히 사용자 가이드). 2026-08-12(사용자 버그 리포트 4개 항목 중 3번, 1차): 그룹·서브카테고리 아코디언 접힘 상태가 세그먼트/스코프 전환 시 유지되지 않던 문제의 1차 수정 — segment/scope 변경 시 접힘 상태를 리셋하는 useEffect 추가(부분 수정, 곧바로 아래 2차로 대체됨). 2026-08-12(같은 항목, 2차·최종): 사용자 재확인 결과 Positive/Negative 자체도(하위 그룹뿐 아니라) 독립적으로 접혀야 관리자 카탈로그 화면과 같은 3단(scope→group→subcategory) 계층이 된다는 지적 — 탭 방식 Positive/Negative 선택(`activeScopeKey` 상태, 이번에 제거)을 폐지하고 `helpers/promptCatalog.ts`의 `promptScopeAccordionKey()`를 사용한 스코프 레벨 아코디언으로 전면 전환. 브라우저에서 Positive·Negative가 각각 독립적으로 접히는 최상위 아코디언으로 렌더되고 하위 그룹·서브카테고리도 기본 접힘 상태로 확인됨.*
...
- [x] `6b` 사용자 매뉴얼 — *커밋 `fa50188`. 구버전 `ManualModal`(오버레이)을 `ManualScreen`(AppShell 본문 전체 화면)으로 전환, iframe 검색 로직 이관. `components/Modals.tsx`는 마지막 export가 빠지며 파일 삭제. 더미 금지로 목차(TOC)·PDF 내려받기 제외. 2026-08-12: 사용자 요청 - `docs/dobedub-studio-user-manual.md` 원본이 v3 이전(모달 기반 GNB) 시절 내용과 캡처 이미지로 채워져 있어 현재 화면과 어긋난다는 지적. 문서를 전면 재작성 - Claude in Chrome으로 로그인 후 실제 화면 16장을 새로 캡처(`docs/manual-assets/v4-*.jpg`: 로그인, Workspace, Task History, Prompt Reuse, Assets, 어드민 역할/사용자/사용자상세/카탈로그 트리(접힘·펼침)/Negative 기본값/워크플로/Sandbox Pod/System Status/Metadata/감사 로그/기능 리소스 매핑), 목차를 실제 라우트 구조(GENERATE 흐름 4단계 → Task History/Prompt Library/Assets → ADMIN 8개 화면 → 권한 코드 요약 → 운영 주의사항/FAQ) 기준으로 재구성했다. 2b/2f/2c/2d(세그먼트 설정·실행 전 확인·진행·결과)는 키프레임 업로드가 파일 피커를 거쳐야 해서(이 세션의 `file_upload` 도구가 세션 폴더 제약으로 사용 불가) 실 화면 캡처 대신 소스 코드(`createScreens.tsx`) 확인 내용을 텍스트로 정확히 반영했다 - 추후 캡처가 가능해지면 이미지만 추가하면 된다. 7a(시스템 프롬프트)는 ADMIN 사이드바에 진입 경로가 없어(2b의 System Prompt 편집 탭과 상태를 공유) 별도 캡처 없이 본문에서 설명만 추가. `/manual` 응답 자체는 새 내용을 정상 반환함을 `fetch(cache:'no-store')`로 확인했지만, 프론트가 캐시 우회 옵션 없이 `fetch()`를 호출해 이미 한 번 방문한 브라우저에서는 새로고침 전까지 이전 응답이 보일 수 있다(기능 결함은 아님, 강력 새로고침 시 해소).*

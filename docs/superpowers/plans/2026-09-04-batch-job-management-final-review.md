# Batch 작업 요청 관리 Implementation Plan - 최종 검증 보완본
...
> **최종 검증 보완 규칙 (2026-09-04):** 이 파일은 원본 계획의 검증 보완본이다. 아래 R-D/R-E의
> claim 소유권, 테스트 시그니처, 공유 복구기 배치 규칙이 원본의 충돌하는 예시보다 우선한다.
...
- Produces: `BatchJob`에 컬럼 5개 추가 — `prompt_waiting_count`, `prompt_generating_count`,
  ...
  `RunpodRequestItem`에 공유 복구 컬럼 2개 — `materialization_claimed_at`,
  `materialization_attempts` (기본 0). 인덱스 4개 — `ix_image_prompt_drafts_batch_status`,
  `ix_workflow_tasks_batch_deleted_status`, `ix_image_prompt_drafts_promotion`,
  `ix_runpod_request_items_orphan`.
...
```python
    ...
    # R-E: 대화형과 배치가 공유하는 request item 복구 lease와 영속 시도 횟수.
    if "runpod_request_items" in tables and "materialization_claimed_at" not in _columns(inspector, "runpod_request_items"):
        op.add_column("runpod_request_items", sa.Column("materialization_claimed_at", sa.DateTime(), nullable=True))
    if "runpod_request_items" in tables and "materialization_attempts" not in _columns(inspector, "runpod_request_items"):
...
```python
    ...
    aggregation_indexes = (
        ...
        # SE-17 복구기 전용. stale claim을 포함해 고아 item 후보를 스캔한다.
...
```python
    for table, index_name, _columns in (
        ...
        ("image_prompt_drafts", "ix_image_prompt_drafts_promotion", None),
        ("runpod_request_items", "ix_runpod_request_items_orphan", None),
    ):
...
같은 파일의 `RunpodRequestItem`에 R-E의 영속 복구 상태를 추가한다:
...
포함하고, `promotion_claimed_at`·`materialization_claimed_at`·`materialization_attempts` 컬럼 제거도 넣는다:
...
```python
    ...
    if "runpod_request_items" in tables and "materialization_claimed_at" in _columns(inspector, "runpod_request_items"):
        op.drop_column("runpod_request_items", "materialization_claimed_at")
    if "runpod_request_items" in tables and "materialization_attempts" in _columns(inspector, "runpod_request_items"):
...
- Produces: `batch_job_service.claim_batch_drafts_for_promotion(db, *, limit) -> tuple[list[tuple[str, str, str]], dict[str, datetime]]`
  — `([(batch_job_id, owner_id, draft_id), ...], {draft_id: persisted_claimed_at})`. 두 번째 값은 DB에서
  다시 읽은 fencing version이다.
- Produces: `batch_job_service.still_owns_claim(db, claims) -> list[str]` — 제출 직전 재확인용
...
def test_claim_is_exclusive_across_independent_sessions(db_session, monkeypatch):
    ...
    try:
        claimed_a, _ = batch_job_service.claim_batch_drafts_for_promotion(session_a, limit=10)
        claimed_b, _ = batch_job_service.claim_batch_drafts_for_promotion(session_b, limit=10)
    finally:
        session_a.close()
        session_b.close()
...
def test_two_threads_racing_the_same_candidates_never_double_claim(db_session, monkeypatch):
    ...
    가드를 제거하면 overlap이 후보 전량으로 터진다. 5/5 결정적이며 오류도 없다.
    """
    import threading
    from backend.app.db.session import SessionLocal
    ...
    def worker(name: str) -> None:
        ...
        try:
            ...
            claimed, _ = batch_job_service.claim_batch_drafts_for_promotion(session, limit=10)
            results[name] = [draft_id for _b, _o, draft_id in claimed]
...
def test_expired_claim_is_not_reclaimed_when_the_request_item_exists(db_session, monkeypatch):
    ...
    claimed, _ = batch_job_service.claim_batch_drafts_for_promotion(db_session, limit=10)
    assert claimed == []


def test_old_owner_cannot_release_a_newer_claim(db_session, monkeypatch):
    """느린 A의 실패 처리가 만료 뒤 재선점한 B의 lease를 지우면 안 된다."""
    from backend.app.db.session import SessionLocal
    ...
    session_a, session_b = SessionLocal(), SessionLocal()
    try:
        _claimed_a, stamps_a = batch_job_service.claim_batch_drafts_for_promotion(session_a, limit=10)
        # A가 멈춘 동안 lease가 만료됐다고 만들고 B가 재선점한다.
        session_b.execute(
            ImagePromptDraft.__table__.update()
            .where(ImagePromptDraft.id == draft.id)
            .values(promotion_claimed_at=datetime.utcnow() - timedelta(seconds=361))
        )
        session_b.commit()
        _claimed_b, stamps_b = batch_job_service.claim_batch_drafts_for_promotion(session_b, limit=10)

        batch_job_service.release_promotion_claim(session_a, stamps_a)
        persisted = session_b.get(ImagePromptDraft, draft.id)
        session_b.refresh(persisted)
        assert persisted.promotion_claimed_at == stamps_b[draft.id]
    finally:
        session_a.close()
        session_b.close()
...
def claim_batch_drafts_for_promotion(
    ...
    db.commit()
    # DB가 저장한 실제 시각을 다시 읽는다. MySQL의 DATETIME 정밀도가 호출 측 now와
    # 다를 수 있으므로, 메모리의 now를 fencing 값으로 재사용하면 안 된다.
    claimed_ids = [draft_id for _batch_id, _owner_id, draft_id in claimed]
    persisted_claims = {
        str(draft_id): claimed_at
        for draft_id, claimed_at in db.execute(
            select(ImagePromptDraft.id, ImagePromptDraft.promotion_claimed_at).where(
                ImagePromptDraft.id.in_(claimed_ids)
            )
        ).all()
        if claimed_at is not None
    }
    return claimed, persisted_claims
...
def still_owns_claim(db: Session, claims: dict[str, datetime]) -> list[str]:
    ...
    The persisted timestamp is a fencing version, not just a liveness marker.
    It protects a later release from clearing a newer owner's claim without
    adding a second token column. A residual race remains between this check
    and the local task creation, so the lease must stay above the measured
    worst-case duration of one PROMOTION_LIMIT_PER_CYCLE promotion call.
    """
    if not claims:
        return []
    owned: list[str] = []
...
def release_promotion_claim(db: Session, claims: dict[str, datetime]) -> None:
    ...
    if not claims:
        return
    for draft_id, claimed_at in claims.items():
        # A slow, expired owner must never release a newer owner's lease.
        db.execute(
            ImagePromptDraft.__table__.update()
            .where(
                ImagePromptDraft.id == draft_id,
...
**Files:**
- Modify: `backend/app/services/batch_job_service.py` (완료 판정만)
- Modify: `backend/app/services/runpod_request_batch_service.py` (claim 조건부 link/fail)
- Create: `backend/app/services/request_item_recovery_service.py`
- Modify: `backend/app/main.py` (monitor 단계 추가)
- Test: `backend/tests/test_durable_runpod_request_batch.py`, `backend/tests/test_batch_job_service.py`
...
### R-E0: 공유 복구기 최종 구현 규칙 (아래 기존 Step 1~3을 대체)

이 복구기는 대화형 RunPod 요청에도 적용하지만, 이 브랜치에서는 `20260904_0033_batch_jobs.py`가
필요한 인덱스를 함께 만들므로 **별도 main cherry-pick 대상으로 표기하지 않는다.** 커밋은 리뷰와
되돌리기 단위로 분리하되, 단독 배포하려면 `main(0032)`에서 새 공유 migration을 만들고 배치 migration을
`0034`로 재번호화하는 별도 작업이 필요하다.

**Files:**
- Modify: `backend/app/db/migrations/versions/20260904_0033_batch_jobs.py`
- Modify: `backend/app/db/models.py` (`RunpodRequestItem`)
- Create: `backend/app/services/request_item_recovery_service.py`
- Modify: `backend/app/main.py`
- Modify: `backend/app/services/batch_job_service.py` (`_batch_is_settled`만)
- Test: `backend/tests/test_durable_runpod_request_batch.py`

`RunpodRequestItem`에 다음 DB 상태를 추가한다. FK는 추가하지 않는다.
...
인덱스는 기존 orphan 인덱스를 `("status", "task_id", "materialization_claimed_at")`로 바꾼다.
`materialization_claimed_at`은 liveness와 fencing version을 겸한다. claim 직후에는 반드시 DB에서
다시 읽은 실제 저장값을 반환하고, release/link/fail 모든 UPDATE는 `item_id + claimed_at` 조건을 쓴다.

`request_item_recovery_service.py`는 아래 계약으로 구현한다.

1. `claim_orphan_request_items(limit)`은 `task_id IS NULL`, `status=PENDING_SUBMIT`, 생성 시각이 cutoff 이전,
   claim이 비었거나 만료된 item을 고른 뒤 조건부 UPDATE의 `rowcount == 1`인 item만 반환한다. claim과
   `materialization_attempts = materialization_attempts + 1`은 같은 UPDATE로 저장한다.
2. 각 claim마다 먼저 `WorkflowTask.request_item_id == item_id`인 task를 찾는다. 있으면 새 task를 만들지 않고
   claim 조건으로 item에 연결한다. 이 순서가 `create_job()` 직후 프로세스가 죽은 경우의 중복 제출을 막는다.
3. task가 없을 때만 claim을 제출 직전에 재확인한 뒤 `create_job()`을 호출한다. `attach_task_to_request_item()`은
   `task_id IS NULL AND materialization_claimed_at == claimed_at` 조건으로 연결하고, 0행이면 새 task를
   dispatcher 대상으로 남기지 않도록 실패 처리 또는 재연결 경로를 즉시 수행한다.
4. 실패는 DB의 `materialization_attempts`를 기준으로 판정한다. 3회 미만이면 **자기 claim만** release하고,
   3회 이상이면 **자기 claim일 때만** FAILED로 확정한다. 프로세스 로컬 dict는 사용하지 않는다.

테스트는 기존 request-batch fixture만 사용하고 BatchJob을 만들지 않는다.

- stale 고아 item이 정확히 한 개의 task로 연결된다.
- 최근 item은 claim되지 않는다.
- Barrier를 둔 두 독립 Session이 같은 item을 놓고 경쟁해도 claim 성공자는 하나다.
- item에 이미 `request_item_id` task가 있으면 `create_job()`을 호출하지 않고 그 task만 연결한다.
- 재시작을 모사한 새 Session에서도 attempt 수가 유지되고, 세 번째 실패에서만 FAILED가 된다.

이 규칙은 원본의 프로세스 로컬 `_MATERIALIZE_ATTEMPTS`와 claim 없는 조회/생성 예시를 완전히 대체한다.
...
- [ ] **Step 5: monitor에 공유 복구 단계 추가**

`backend/app/main.py`의 `monitor_loop`에서 기존 프롬프트 generation 단계 다음에 넣는다.
배치 승격의 존재 여부와 관계없이 대화형 요청도 복구해야 하므로, `batch_job_service`가 아니라
`request_item_recovery_service`에서 import한다. G-3에 따라 독립 try/except로 감싼다:
...
import에 `request_item_recovery_service.materialize_orphan_request_items`를 추가한다.
...
```bash
python3 -m pytest backend/tests/test_durable_runpod_request_batch.py backend/tests/test_runpod_submission_queue.py -rf
python3 -m pytest backend/tests/test_batch_job_service.py -rf -k "settles"
python3 -m compileall -q backend/app
```
Expected: 전부 PASS. 두 번째 명령이 중요하다 — 복구기는 대화형 경로의 item에도 작동하므로,
정상 흐름의 item을 가로채지 않는지 여기서 확인된다.

- [ ] **Step 7: 커밋 — 논리적으로 두 개로 나눈다**

복구기는 **대화형 경로의 기존 결함도 고치므로 배치 완료 판정식과 섞지 않는다.** 다만 이 브랜치에서는
0033 migration에 의존하므로 이 커밋을 `main`에 단독 cherry-pick할 수 있다고 주장하지 않는다:

```bash
# 1) 공유 결함 수정 — 배치 완료 판정식과 분리
git add backend/app/db/migrations/versions/20260904_0033_batch_jobs.py backend/app/db/models.py backend/app/services/request_item_recovery_service.py backend/app/services/runpod_request_batch_service.py backend/app/main.py backend/tests/test_durable_runpod_request_batch.py
...
> 브랜치를 쪼개는 대신 **커밋을 쪼개** 리뷰·되돌리기 경계를 확보한다. 공유 복구기만 별도 선행 배포하려면
> `main`에서 독립 migration을 먼저 만들고 배치 migration을 그 뒤 revision으로 재정렬한다.

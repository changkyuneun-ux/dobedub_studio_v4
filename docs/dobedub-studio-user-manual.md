...
| 2026-08-14 | 프롬프트 평가 기준과 재사용 목록의 사유 표시가 일관되지 않음 | **평가 기준을 원본 유지·자연스런 동작·왜곡 없음·배경 안정·색감 안정으로 통일.** 등급과 평가 근거(사유 또는 코멘트)를 필수로 하고, 라이브러리는 번호 목록으로 표시 | 5, 6 |
| 2026-08-13 | Assets 화면의 새 컬렉션 입력이 상단 헤더에 섞여 눈에 잘 띄지 않음 | **자산 목록 직전의 독립 `컬렉션 관리` 영역으로 이동.** 현재 컬렉션 수, 새 컬렉션 이름 입력, 생성 버튼을 한 영역에서 제공 | 7 |
| 2026-08-13 | 컬렉션 삭제 시 분류된 자산 연결이 함께 사라질 수 있는 경로가 없음 | **빈 컬렉션만 삭제하도록 변경.** 분류된 자산이 남아 있으면 자산 수와 함께 분류 변경/해제 후 삭제하라는 안내를 표시하고, 서버 API도 삭제를 차단 | 7 |
| 2026-08-13 | Qwen 서버리스 콜드 스타트에서 첫 프롬프트 요청이 HTTP 대기시간을 넘으면 실패한 것처럼 보임 | **비동기 Qwen 요청 추적으로 변경.** 요청은 서버에 저장되고 RunPod Job 상태를 최대 15분 동안 확인하므로, 브라우저 요청·화면 전환과 분리되어 완료 또는 실패로 정리 | 4.2, 20, 21 |
| 2026-09-11 | 웹툰 원본 컷을 I2V 입력용 이미지로 준비하는 절차가 외부 도구에 의존 | **LOCAL `이미지 컷 분할` 추가.** ECS/로컬 서버는 화면 코드와 정책만 제공하고, PDF·이미지·ZIP 컷 분할은 사용자의 브라우저 로컬 파일 시스템에서만 수행 | 3, 4 |
...
`이미지 컷 분할`은 `GENERATE` 위의 독립 `LOCAL` 영역에 표시됩니다. 이 기능은 RunPod/ECS 작업 큐에 제출하는 영상 생성 기능이 아니라, 향후 I2V 입력에 사용할 컷 PNG를 사용자의 컴퓨터에서 직접 만드는 로컬 처리 기능입니다.

- 지원 브라우저: 데스크톱 Chrome·Edge 최신 버전
- 권한: 메뉴 노출은 `jobs:run` 권한을 사용하지만, 파일 읽기·쓰기는 운영체제와 브라우저의 로컬 디렉토리 권한으로 제한됩니다. 입력 폴더는 읽기 권한만, 작업 폴더는 출력 생성을 위한 쓰기 권한만 요청합니다.
...
5. 다른 DOBEDUB 화면으로 이동해도 같은 탭에서는 Web Worker 작업과 진행 상태를 유지합니다. 새로고침 또는 탭 닫기 후 재진입한 경우에는 저장된 handle 권한과 `manifest.json`을 확인해 이미 검증된 PNG는 건너뛰고 누락 단위만 이어서 처리합니다.
...
- `fullpage`: 컷 경계를 찾지 못해 전체 페이지/이미지를 1컷으로 저장했습니다.
- `review_continuous`: 흰 여백은 없지만 긴 연속 장면이라 수평 장면 전환 후보 검수가 필요합니다.
- `thin`, `many`: 너무 얇거나 지나치게 많은 컷이 감지되어 사람이 확인해야 합니다.
- `missing_output`, `error`: inventory에는 있으나 검증된 PNG가 없거나 처리 오류가 발생했습니다. `실패/누락 재시도`로 해당 단위만 다시 처리합니다.

수평 장면 전환은 기본 처리에서 자동 적용하지 않습니다. `review_continuous` 항목을 선택한 뒤 `수평 장면 전환` 재처리 유형을 고르면 다음 조건을 만족하는 선만 적용합니다.

- 긴 `continuous_sequence` 결과에만 적용
- 전체 폭 95% 이상에서 명암·색상이 급변
- 최소 컷 높이와 경계 간 거리 조건을 만족해 액자선·장식선을 제외

재처리로 긴 컷이 여러 컷으로 나뉘면 기존 긴 컷 PNG는 출력 폴더에서 삭제되고, 새 분할 PNG와 갱신된 `manifest.json`/`summary.csv`만 남습니다.
...
#### Qwen LLM 프롬프트 생성과 상태 확인

`프롬프트 생성 · Qwen`은 영상 생성용 ComfyUI와 **별도의 RunPod Serverless Qwen 서비스**를 사용합니다. 따라서 영상 작업이 실행 중이더라도 프롬프트 초안 생성은 독립적으로 요청할 수 있습니다.

1. Positive Key word 또는 Scene Detail을 입력한 뒤 `프롬프트 생성 · Qwen`을 누릅니다.
2. 앱은 요청을 먼저 저장하고 Qwen Serverless에 제출합니다. 워커가 꺼져 있으면 `Qwen 워커를 준비하고 있습니다`가 표시됩니다.
3. 워커가 준비되면 `Qwen이 프롬프트를 생성하고 있습니다` 상태로 바뀝니다. 콜드 스타트는 일반 생성보다 오래 걸릴 수 있습니다.
4. 생성이 완료되면 Draft의 Positive/Negative Prompt를 검토합니다. **이 시점에는 세그먼트에 아직 적용되지 않습니다.** `Apply Generated Prompt`를 눌러야 적용됩니다.
5. 브라우저 새로고침, Prompt Builder 닫기, 다른 화면 이동으로 요청이 취소되지는 않습니다. 다시 Prompt Builder를 열면 서버에 기록된 최종 상태를 확인할 수 있습니다.

Qwen 요청은 제출 시점부터 최대 15분 동안 서버가 상태를 확인합니다. 이 시간 안에 RunPod가 완료를 반환하지 않거나 RunPod가 `FAILED`/`CANCELLED`를 반환하면 실패 사유가 화면에 표시됩니다. 실패해도 직접 입력, `Apply Keyword / Scene Draft`, Prompt Library 재사용은 계속 사용할 수 있습니다.
...
- 목록 컬럼: 워크플로, 시작→다음 이미지 썸네일, 프롬프트(Positive/Negative, 스크롤 가능), 평가 사유, 코멘트, 레이팅, 생성자, 모델명, `재사용` 버튼.
- 상단 검색창에 프롬프트·코멘트·사유·task ID로 검색할 수 있습니다.
- 서버사이드 페이지네이션으로 동작하며, 화면 진입 시 목록이 자동으로 로드됩니다.
- 재사용 후보는 Task History의 Prompt Review 아코디언에서 품질 등급과 평가 근거를 저장하고 "재사용 가능"으로 등록한 프롬프트입니다. **등급 선택은 필수**이며, 평가 사유(원본 유지·자연스런 동작·왜곡 없음·배경 안정·색감 안정) 중 하나 이상을 선택하거나 코멘트를 입력해야 저장할 수 있습니다. 라이브러리에서는 선택된 사유를 번호 목록으로 확인합니다.
...
- Asset 행의 `+ 컬렉션에 담기` 목록에서 대상 컬렉션을 선택합니다. 이미 분류된 컬렉션은 칩으로 표시되고, 칩 오른쪽 `×`로 해당 분류만 해제합니다. Asset 자체와 원본 파일은 삭제되지 않습니다.
- `컬렉션 관리` 영역에는 각 컬렉션의 자산 수와 `삭제` 버튼이 함께 표시됩니다. **자산 수가 0개인 빈 컬렉션만 삭제할 수 있습니다.** 분류된 자산이 남아 있으면 삭제가 차단되며, 먼저 각 Asset 행에서 다른 컬렉션으로 옮기거나 `×`로 해당 분류를 해제해야 합니다. 컬렉션 삭제는 Asset·원본 파일·다른 컬렉션 분류를 삭제하지 않습니다.
...
컬렉션은 현재 접속한 환경의 DB에 저장됩니다. 로컬 개발 DB와 운영 ECS/RDS의 컬렉션 데이터는 자동으로 서로 복사되지 않습니다. 운영 화면에서 새로 만든 컬렉션은 운영 자산에만, 로컬에서 만든 컬렉션은 로컬 자산에만 표시됩니다.
...
- **Qwen 콜드 스타트는 정상 대기 상태일 수 있습니다.** `Qwen 워커를 준비하고 있습니다`는 즉시 실패가 아닙니다. 앱이 최대 15분 동안 RunPod Job 상태를 추적하며, 완료되면 Draft를 표시하고 실패하면 원인을 표시합니다.
...
| Qwen Prompt가 생성되지 않음 | System Status에서 Qwen 상태를 확인하고, Scene Detail 또는 Positive 키워드가 하나 이상 있는지 확인하세요. `Qwen 워커를 준비하고 있습니다`는 콜드 스타트 대기 상태이며 최대 15분 동안 자동 추적됩니다. `FAILED` 또는 `CANCELLED`가 표시되면 안내된 실패 사유를 확인하세요 |
